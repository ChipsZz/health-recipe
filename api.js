...
async function generateRecipe(request, env) {
    try{
        ...
        // 1.获取用户信息
        // const userProfile = await env.DB.prepare('SELECT goal FROM users WHERE id = ?').bind(user_id).first();
        // const healthGoal = goal;

        // 2.将查询转换为向量并从Vectorize检索相关知识
        ...
        const prompt = `
          ...
          ## Example:
          - User inputs in Chinese → Reply in Simplified Chinese
          - User inputs in English → Reply in English
          - User inputs in Japanese → Reply in Japanese
...
async function foodUpload(request, env) {
  try{
      ...
      const input = {
          ...
          prompt: "What foods are in this image, and how many of each type are there? If there is no food, just return 'None'.",
          max_tokens: 512,
      };
      ...
      console.log("none: " + JSON.stringify(response));
      ...
      const prompt = `
          ...
          2. Duplicate items should be combined.
      `;
